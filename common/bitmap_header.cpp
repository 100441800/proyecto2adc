...
template <typename T, typename S> T get_value(S buffer, int offset) noexcept {
...
bitmap_header::bitmap_header(int w, int h) noexcept
    : pixel_start_{header_size}, width_{w}, height_{h} {
...
void bitmap_header::read(std::istream &is) {
  using enum file_error_kind;
  read_buffer(is);
  const std::span header_view{header_info};
  check_file_error(header_view[0] == 'B' and header_view[1] == 'M',
