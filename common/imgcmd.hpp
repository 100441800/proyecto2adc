...
void process_image(image_type &image,
...
template <typename image_type>
auto generate_output(const std::filesystem::path &out_dir,
  ...
  auto process_time = clk::now();
  std::ofstream histogram_out{out_dir /
                              in_file.filename().replace_extension(".hst")};
...
void print_times(auto times) noexcept {
  ...
  std::cout << "  Process time: "
            << duration_cast<microseconds>(times[2]).count() << '\n';
  std::cout << "  Store time: " << duration_cast<microseconds>(times[3]).count()
            << '\n';
}

template <typename image_type>
void process_file(const std::filesystem::path &in_file,
  ...
  try {
    ...
    const auto [read_time, process_time, write_time] =
        generate_output<image_type>(out_dir, in_file, subcmd);
