...
void normalized_pixel::intensity_transform() noexcept {
  for (auto &c : color) {
    c = (c <= intensity_threshold)
            ? c / intensity_divisor1
            : std::pow((c + intensity_delta) / intensity_divisor2,
                       intensity_exponent);
  }
}

double normalized_pixel::to_gray() const noexcept {
  return red_coefficient * red() + green_coefficient * green() +
         blue_coefficient * blue();
}

double gamma_correction(double g) noexcept {
  return (g <= gamma_threshold)
             ? intensity_divisor1 * g
             : intensity_divisor2 * std::pow(g, intensity_expornent_inv) -
                   intensity_delta;
}
