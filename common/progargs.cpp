...
const std::map<std::string_view, subcommand> subcommand_map{
    // NOLINT(cert-err58-cpp)
    {"copy"sv, subcommand::copy}, {"histo"sv, subcommand::histo},
    {"mono"sv, subcommand::mono}, {"gauss"sv, subcommand::gauss},
...
void error_invalid_argument(std::ostream &os, std::string_view prog_name,
...
void error_input_missing(std::ostream &os, std::string_view prog_name,
                         std::string_view in, std::string_view out) noexcept {
  os << "Input path: " << in << "\n";
  os << "Outut path: " << out << "\n";
...
void error_output_missing(std::ostream &os, std::string_view prog_name,
                          std::string_view in, std::string_view out) noexcept {
  os << "Input path: " << in << "\n";
  os << "Outut path: " << out << "\n";
...
configuration parse_arguments(const std::vector<std::string> &args) noexcept {
  ...
  if (!op) {
    error_invalid_argument(std::cerr, args[0], args[3]);
  } else {
    subcmd = *op;
  }
